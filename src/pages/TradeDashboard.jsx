import { ArrowRightLeft, Clock, CheckCircle, XCircle, RefreshCw, MessageCircle } from 'lucide-react'
...
export default function TradeDashboard() {
    ...
    const fetchTrades = useCallback(async () => {
        ...
        setError('')
        try {
            // incoming = seller_id = me (someone wants to trade for MY item)
            // outgoing = buyer_id = me  (I proposed a trade)
            const filterCol = tab === 'incoming' ? 'seller_id' : 'buyer_id'

            const { data, error: err } = await supabase
                .from('trades')
                .select(`
                    id, status, offer_item_desc, message, created_at, updated_at,
                    item:items!trades_item_id_fkey(id, title, images),
                    buyer:profiles!trades_buyer_id_fkey(id, full_name, uid),
                    seller:profiles!trades_seller_id_fkey(id, full_name, uid)
                `)
